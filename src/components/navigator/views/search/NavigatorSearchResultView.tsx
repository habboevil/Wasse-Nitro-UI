    ...
    return (
        <Column className="bg-white rounded border border-muted" gap={ 0 }>
            <Flex fullWidth alignItems="center" justifyContent="between" className="px-2 py-1">
                ...
                <>
                    {
                        gridHasTwoColumns ? <AutoGrid columnCount={ 3 } { ...rest } columnMinWidth={ 200 } columnMinHeight={ 130 } className="mx-2" >
                            { searchResult.rooms.length > 0 && searchResult.rooms.map((room, index) => <NavigatorSearchResultItemView style={ { height: 'auto', margin: '3px', display: 'inline', } } key={ index } roomData={ room } thumbnail={ true } />) }
