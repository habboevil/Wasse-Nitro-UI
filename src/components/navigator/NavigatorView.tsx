        ...
        <>
            { isVisible &&
                <NitroCardView uniqueKey="navigator" className="nitro-navigator">
                    <div style={ { background: 'url("https://habbolatam.com/wasseyemma/navegador_1.gif") 0px -15px', } }>
                        <NitroCardHeaderView
                            headerText={ LocalizeText(isCreatorOpen ? 'navigator.createroom.title' : 'navigator.title') }
                            onCloseClick={ (event) => setIsVisible(false) }
                            style={ { height: '45px', } }
                        />
                        <NitroCardTabsView style={ { marginTop: '68px', backgroundColor: '#fff0', } }>
                            { topLevelContexts && topLevelContexts.length > 0 && topLevelContexts.map((context, index) => (
                                (context.code !== 'roomads_view') && // Agrega esta condición para excluir la sección específica
  <NitroCardTabsItemView
      key={ index }
      style={ { borderRadius: '0px', fontSize: '13px', position: 'relative' } }
      isActive={ topLevelContext === context && !isCreatorOpen }
      onClick={ (event) => sendSearch('', context.code) }
  >
      <img
          src={ `https://habbolatam.com/salas/${ context.code }.png` }
          alt={ `${ context.code }` }
          style={ {
              width: '20px',
              height: '20px',
      ...
      { LocalizeText(`navigator.toplevelview.${ context.code }`) }
                    ...
                    <NitroCardContentView position="relative" style={ { marginTop: '0px', } }>
                        { isLoading &&
