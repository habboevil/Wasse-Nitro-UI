...
export const ModToolsRoomView: FC<ModToolsRoomViewProps> = props =>
{
    ...
    return (
        <NitroCardView className="nitro-mod-tools-room" theme="primary-slim" windowPosition={ DraggableWindowPosition.TOP_LEFT }>
            <NitroCardHeaderView headerText={ 'Información' + (name ? ': ' + name : '') } onCloseClick={ event => onCloseClick() } />
            <NitroCardContentView className="text-black">
                <Flex gap={ 2 }>
                    <Column justifyContent="center" grow gap={ 1 }>
                        <Flex alignItems="center" gap={ 2 }>
                            <Text bold align="end" className="col-7">Dueño de Sala:</Text>
                        ...
                        <Flex alignItems="center" gap={ 2 }>
                            <Text bold align="end" className="col-7">Dueño en Sala:</Text>
                            <Text>{ ownerInRoom ? 'Yes' : 'No' }</Text>
                        </Flex>
                    </Column>
                    <Column gap={ 1 }>
                        <Button onClick={ event => TryVisitRoom(roomId) }>Ir a la sala</Button>
                ...
                <Column className="bg-muted rounded p-2" gap={ 1 }>
                    ...
                    <Flex alignItems="center" gap={ 1 }>
                        ...
                        <Text small>Cambiar el nombre de sala</Text>
                    </Flex>
                </Column>
                <textarea className="form-control" placeholder="Escriba un mensaje obligatorio para los usuarios en este cuadro de texto..." value={ message } onChange={ event => setMessage(event.target.value) }></textarea>
                <Flex justifyContent="between">
                    <Button variant="danger" onClick={ event => handleClick('send_message') }>Enviar Advertencia</Button>
                    <Button onClick={ event => handleClick('alert_only') }>Sólo enviar alerta</Button>
                </Flex>
